...
public class WithdrawalViewModel extends BaseViewModel<JsonResponse<List<String>>> {
     ...
     * @param name
     */
    public void incBindBank(String bankNumber,String bankName,String name) {
        HashMap map = new HashMap();

        map.put("depositBank",bankName);//开户行
        map.put("payAccount",bankNumber);//账户
        map.put("payAccountType","2");// 账户类型 1个人 2公司
        map.put("payAccounthouseholder",name);//账户名
        map.put("payType","3");
        mSubscriber = getBindSubscriber();
        mServer.getIncBindBankChat(mSubscriber,map);
    }
