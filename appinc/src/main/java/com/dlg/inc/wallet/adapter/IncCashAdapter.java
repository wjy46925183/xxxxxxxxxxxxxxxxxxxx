 ...
 */
public class IncCashAdapter extends BaseAdapter {
	...
	public IncCashAdapter(Context context, List<BindBean> bindBeens, CheckBoxChangeListener listener) {
	...
	public IncCashAdapter(Context context, List<BindBean> bindBeens) {
		this.bindBeens = bindBeens;
		mContext = context;
		inflater = LayoutInflater.from(context);
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.inflate(R.layout.inc_item_cash_bind, parent, false);
		...
		int payType = bindBeens.get(position).getPayType();
		...
		if (payType == 1) {      //支付宝
			...
			if (null != bean.getPayAccountType() && bean.getPayAccountType().toString().equals("2")) {//企业账户
//                if (bean.getPayAccount() != null && !"".equals(bean.getPayAccount())){
				viewHolder.card_user_name
						.setText(bean.getPayAccount());
...
//
//                }
			} else if (null != bean.getPayAccountType() && bean.getPayAccountType().toString().equals("1")) { //银行
//                if (bean.getPayAccount() != null && !"".equals(bean.getPayAccount())){
				viewHolder.card_user_name
						.setText(bean.getPayAccount());
...
//                }
			...
			viewHolder.card_user_name.setText("未绑定");
			viewHolder.img_icon.setImageBitmap(BitmapFactory.decodeResource(mContext.getResources(), R.mipmap.zhifu_ka));
			viewHolder.tv_bind_name.setText("公司账户");
			viewHolder.check_type.setVisibility(View.GONE);
			viewHolder.img_to_bind.setVisibility(View.VISIBLE);
		}
		//企业账户，目前1对应支付宝 2对应微信   3 对应银行卡  这边添加个判断即可，并将对应的图标修改
