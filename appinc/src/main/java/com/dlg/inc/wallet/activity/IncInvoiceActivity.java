...
import com.dlg.inc.wallet.adapter.IncInvoiceListAdapter;
 ...
 */
public class IncInvoiceActivity extends IncBaseActivity implements InvoicePresenter,BaseLoadMoreHeaderAdapter.OnLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
	...
	private IncInvoiceListAdapter mInvoiceListAdapter;//适配器
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		setContentView(R.layout.inc_page_invoices, IncToolBarType.Default);
	...
	private void initData() {
		...
		mToolBarHelper.setToolbarTextRightOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				ActivityNavigator.navigator().navigateTo(IncInvoiceHistoryActivity.class);
			}
		});

		mInvoiceListAdapter = new IncInvoiceListAdapter(this,mLvInvoiceListing,mInvoiceListBeanData, R.layout.inc_item_invoice_list);
		...
		mNextBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (isNext){
					...
					ActivityNavigator.navigator().navigateTo(IncOrderBillActivity.class,bunder,REQUEST_ORDER);
				}
			}
		});
	}
	...
	private void setNext(boolean b) {
		isNext = b;
		if (b){
			mNextBtn.setBackgroundResource(R.color.inc_color_ff7a51);
		}else {
			mNextBtn.setBackgroundResource(R.color.inc_app_color_gainsboro);
		}
	}
