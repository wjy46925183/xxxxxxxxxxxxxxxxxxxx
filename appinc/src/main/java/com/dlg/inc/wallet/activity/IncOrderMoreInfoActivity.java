...
public class IncOrderMoreInfoActivity extends IncBaseActivity {
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		setContentView(R.layout.inc_page_order_more_info);
		initView();
		initData();
		initListener();
	}
	...
	private void submit() {
		...
		if (taxpayerDistinguishNumber.isEmpty()){
			IncToastUtils.showShort(this,"纳税人识别号不能为空");
			return;
		}
		if (companyAddress.isEmpty()){
			IncToastUtils.showShort(this,"地址不能为空");
			return;
		}
		if (companyPhone.isEmpty()){
			IncToastUtils.showShort(this,"电话不能为空");
			return;
		}
		if (bankAccount.isEmpty()){
			IncToastUtils.showShort(this,"开户行不能为空");
			return;
		}
		if (bankAccountNumber.isEmpty()){
			IncToastUtils.showShort(this,"开户行账号不能为空");
			return;
		}
