...
import com.dlg.inc.view.IncTextOrEditxtView;
...
public class IncPaymentActivity extends IncBaseActivity implements  PaymentPresenter, DictionaryPresenter {
	private ScrollView mScrollview;
	private IncTextOrEditxtView mTvPayCompensation;// 报酬
	private IncTextOrEditxtView mTvPayTip;//小费
	private IncTextOrEditxtView mTvPayInsurance;//保险
	private IncTextOrEditxtView mTvPayServiceCharge;//平台服务费
	private IncTextOrEditxtView mTvPayAmount;//总金额
	private IncTextOrEditxtView mTvPayBalance;//余额
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		setContentView(R.layout.inc_page_payment);
		initView();
		initData();
		initNet();
		initListener();
	...
	private void initView() {
		...
		mTvPayCompensation = (IncTextOrEditxtView) findViewById(R.id.tv_pay_compensation);
		mTvPayTip = (IncTextOrEditxtView) findViewById(R.id.tv_pay_tip);
		mTvPayInsurance = (IncTextOrEditxtView) findViewById(R.id.tv_pay_insurance);
		mTvPayServiceCharge = (IncTextOrEditxtView) findViewById(R.id.tv_pay_service_charge);
		mTvPayAmount = (IncTextOrEditxtView) findViewById(R.id.tv_pay_amount);
		mTvPayBalance = (IncTextOrEditxtView) findViewById(R.id.tv_pay_balance);
	...
	public void getPayOKOrNo(String bean) {
		...
		if (bean.equals("0")){
			LogUtils.d("支付成功");
			IncToastUtils.showShort(this,"支付成功");
			finish();
			/**
			 * 保留哪几个
			 */
			...
			//stack.add(IncOddActivity.class);
			ActivityNavigator.navigator().keepRemoverActivity(stack);

		}else {
			LogUtils.d("支付失败");
			IncToastUtils.showShort(this,"支付失败");
		}
	}
