...
public class OrderBillActivity extends BaseActivity implements OrderBillPresenter, DictionaryPresenter {
	...
	private void initListener() {
			...
			public void onClick(View v) {
				...
				if (bean!=null){
					...
					if (orderBillMoreBean!=null){
						...
						ActivityNavigator.navigator().navigateTo(OrderMoreInfoActivity.class,intent,MORE_REQUEST);
		...
		mTvBtnPayment.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				ActivityNavigator.navigator().navigateTo(PublicInputPwdActivity.class,bunder,PWD_REQUEST);
			}
		});
	}
	...
	private void submit() {
		...
		if(TextUtils.isEmpty(companyRise)){
			ToastUtils.showShort(this, "公司抬头不能为空");
			return;
		}
		if(billnoMoney<=0){
			ToastUtils.showShort(this, "发票金额不能少于0");
			return;
		}
		if(TextUtils.isEmpty(addressee)){
			ToastUtils.showShort(this, "收件人不能为空");
			return;
		}
		if(TextUtils.isEmpty(locationArea)){
			ToastUtils.showShort(this, "所在地区不能为空");
			return;
		}
		...
		if(TextUtils.isEmpty(bankaccountAddress)){
			ToastUtils.showShort(this, "开户行地址不能为空");
			return;
		}

		if(orderBillMoreBean==null){
			ToastUtils.showShort(this, "请填写更多信息");
			return;
		}
