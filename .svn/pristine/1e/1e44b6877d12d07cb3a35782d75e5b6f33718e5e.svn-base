...
public class PasswordInputView extends EditText {
    ...
    private static final int defaultSplitLineWidth = 20;

    private int borderColor = 0xFFe8eef6;
    private float borderWidth = 1;
    private float borderRadius = 1;

    private int passwordLength = 6;
    private int passwordColor = 0xFFCCCCCC;
    private float passwordWidth = 6;
    private float passwordRadius = 1;
    ...
    public PasswordInputView(Context context, AttributeSet attrs) {
        ...
        borderColor = a.getColor(R.styleable.PasswordInputView_pivBorderColor,
                borderColor);
        borderWidth = a.getDimension(
                R.styleable.PasswordInputView_pivBorderWidth, borderWidth);
        borderRadius = a.getDimension(
                R.styleable.PasswordInputView_pivBorderRadius, borderRadius);
        passwordLength = a
                .getInt(R.styleable.PasswordInputView_pivPasswordLength,
                        passwordLength);
        passwordColor = a.getColor(
                R.styleable.PasswordInputView_pivPasswordColor, passwordColor);
        passwordWidth = a.getDimension(
                R.styleable.PasswordInputView_pivPasswordWidth, passwordWidth);
        passwordRadius = a
                .getDimension(R.styleable.PasswordInputView_pivPasswordRadius,
                        passwordRadius);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        borderPaint.setColor(0xFFC9D3DF);
        ...
        for (int i = 1; i < passwordLength; i++) {
            float x = (width * i / passwordLength) - 10;
            canvas.drawLine(x, 0, x, height, borderPaint);
        }
        for (int i = 1; i < passwordLength; i++) {
            float x = (width * i / passwordLength) + 10;
