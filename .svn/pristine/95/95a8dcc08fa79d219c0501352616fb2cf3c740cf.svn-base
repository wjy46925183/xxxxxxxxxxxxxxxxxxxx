package com.common.dialog.popup;
...
public abstract class BasePopupWindow implements BasePopup {
    ...
    public BasePopupWindow(Activity context) {
        initView(context, ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT, true);
    }

    public BasePopupWindow(Activity context, int w, int h) {
        initView(context, w, h, true);
    }


    public BasePopupWindow(Activity context, int w, int h, boolean isFocusable) {
        initView(context, w, h, isFocusable);
    }

    private void initView(Activity context, int w, int h, boolean isFocusable) {
        mContext = context;
        ...
        mPopupWindow.setOutsideTouchable(isFocusable);
        //无需动画
    ...
    private void tryToShowPopup(int res, View v) throws Exception {
        ...
        //什么都没传递，取顶级view的id
        if (res == 0 && v == null) {
            mPopupWindow.showAtLocation(mContext.findViewById(android.R.id.content), Gravity.TOP, 0, 0);
        }
        if (curAnima != null && mAnimaView != null) {
            mAnimaView.clearAnimation();
            mAnimaView.startAnimation(curAnima);
