...
public class SettingActivity extends BaseActivity implements View.OnClickListener, LogOutPresenter {
    ...
    private void initView() {
        ...
        mTvCache.setText((ACache.get(this).getCacheSize() / 1024) + "KB");
    }
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.ll_changepwd) {
            ...
            UserAttributeInfoBean userInfo = (UserAttributeInfoBean) mACache.getAsObject(AppKey.CacheKey.MY_USER_INFO);
            String userPhone = mACache.getAsString(AppKey.CacheKey.USER_PHONE);
            String name = mACache.getAsString(AppKey.CacheKey.NAME);
            String userLogo = mACache.getAsString(AppKey.CacheKey.USER_LOGO);
            String userRole = mACache.getAsString(AppKey.CacheKey.USER_ROLE);
            mACache.clear();
            mACache.put(AppKey.CacheKey.MY_USER_ID, userId);
            mACache.put(AppKey.CacheKey.MY_USER_INFO, userInfo);
            mACache.put(AppKey.CacheKey.USER_PHONE, userPhone);
            mACache.put(AppKey.CacheKey.NAME, name);
            mACache.put(AppKey.CacheKey.USER_LOGO, userLogo);
            mACache.put(AppKey.CacheKey.USER_ROLE, userRole);
            ...
            alertView = new AlertView("提示", "确定退出登录？", "取消", null,
                ...
                public void onItemClick(Object o, int position) {
                    dialog = DialogUtils.showLoadingDialog(SettingActivity.this);
                    if (position != -1) {
