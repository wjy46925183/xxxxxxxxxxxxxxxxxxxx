...
public class WithDrawalSource implements WithDrawalInteractor{
    ...
    public Observable<JsonResponse<List<String>>> getIncBindWeChat(HashMap map) {
        return OkGo.post(WalletUrl.INC_PAY_BIND_ADD)//
                .params(map)
                .getCall(new JsonConvert<JsonResponse<List<String>>>() {
                }, RxAdapter.<JsonResponse<List<String>>>create())
                .doOnNext(saveToCacheAction(WalletUrl.INC_PAY_BIND_ADD + JSON.toJSONString(map)))
                .observeOn(AndroidSchedulers.mainThread());
    }
    ...
    public Observable<JsonResponse<List<BankBean>>> getBank(HashMap map) {
        return OkGo.post(WalletUrl.GET_BANK)//
                .params(map)
                .getCall(new JsonConvert<JsonResponse<List<BankBean>>>() {
                }, RxAdapter.<JsonResponse<List<BankBean>>>create())
                .doOnNext(saveToCacheAction(WalletUrl.GET_BANK + JSON.toJSONString(map)))
                .observeOn(AndroidSchedulers.mainThread());
    }
}
