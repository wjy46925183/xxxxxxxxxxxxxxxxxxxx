...
import static gongren.com.dlg.login.activity.LoginActivity.TAG_RESET;
import static gongren.com.dlg.login.activity.LoginActivity.TAG_VERYCODE;
...
public class PersonLoginFragment  extends BaseFragment implements View.OnClickListener, LogInPresenter,TextWatcher {
    ...
    public void initView(View view) {
        psd_text = (EditText) view.findViewById(com.dlg.personal.R.id.psd_text);
        phoneText = (TextView) view.findViewById(com.dlg.personal.R.id.phone_text);
        btnNext = (TextView) view.findViewById(com.dlg.personal.R.id.btnNext);
        forgetPsd = (TextView) view.findViewById(com.dlg.personal.R.id.forget_psd);
        eye = (CheckBox) view.findViewById(com.dlg.personal.R.id.eye);
        verifycodeLogin = (TextView) view.findViewById(com.dlg.personal.R.id.verifycode_login);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == com.dlg.personal.R.id.forget_psd) {//忘记密码
            ...
            addFragment(com.dlg.personal.R.id.frameLayout, resetPsdFragment, TAG_RESET);

        }
        if (v.getId() == com.dlg.personal.R.id.verifycode_login) {//验证码登录
            ...
            addFragment(com.dlg.personal.R.id.frameLayout, certificateFragment, TAG_VERYCODE);
        }
    }

    @Override
    public void logInUserInfo(UserAttributeInfoBean infoBean) {
        mACache.put(AppKey.CacheKey.MY_USER_ID, infoBean.getUserId());//用户ID
        mACache.put(AppKey.CacheKey.NAME, infoBean.getName());//实名
        mACache.put(AppKey.CacheKey.VERIFY_STATE, infoBean.getAuditStatus());//认证状态(0.未审核,1.审核中,2.审核通过,3.审核失败)'
        mACache.put(AppKey.CacheKey.USER_LOGO, infoBean.getLogo());//LOGO
        mACache.put(AppKey.CacheKey.NAME_NUMBER,infoBean.getCertificateNumber());//身份证号

        Intent intent=new Intent(getActivity(),HomeActivity.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        intent.addFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP);
        startActivity(intent);
        JPushInterface.setAliasAndTags(mContext,infoBean.getUserId(),null,new JPushTagAliasCallBack());
    }
